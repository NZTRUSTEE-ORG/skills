...
- Original skill declares Claude-style `allowed-tools`; OpenClaw does not rely on that field inside the skill file.
...
**Path resolution:** All bundled reference docs for this converted skill live in `references/`.
When sub-skills or agents reference `ads/references/*.md`, resolve them to the local `references/` directory in this skill.
