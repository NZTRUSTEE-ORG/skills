...
- Original skill references Claude-specific path conventions such as `.claude/` or `~/.claude/`.
...
Output styles define how the assistant communicates. Styles are bundled as reference files within this skill under `references/` (for example `references/style-conversation.md`, `references/style-email.md`, `references/style-report.md`). Users can override or add custom styles in their local workspace style directory if they maintain one.
...
2. **Fall back to reference**: Otherwise, read the matching style file from this skill's `references/` directory (for example `references/style-email.md` or `references/style-report.md`)
...
- Use normal markdown links or wiki links according to the active workspace conventions; do not assume a literal local file named `links` exists.
